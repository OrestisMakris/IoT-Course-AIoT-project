...
def filter_instances(
    ...
    for item in instances_list:
        # Exclude the last column (labels) from filtering
        filtered_instance = item.iloc[:, :-1].apply(apply_filter, args=(order, wn, filter_type))
        # Concatenate the labels column back to the filtered DataFrame
        filtered_instance = pd.concat([filtered_instance, item.iloc[:, -1]], axis=1)
...
def align_and_combine_data(gyro_df, accel_df):
    """Align and combine gyroscope and accelerometer data.

    This function takes two pandas DataFrames containing gyroscope and accelerometer data,
    aligns them based on the nearest elapsed time, and combines them into a single DataFrame.
    
    Args:
        gyro_df (pd.DataFrame): DataFrame containing gyroscope data with columns 'elapsed (s)', 
                                'x-axis (deg/s)', 'y-axis (deg/s)', and 'z-axis (deg/s)'.
        accel_df (pd.DataFrame): DataFrame containing accelerometer data with columns 'elapsed (s)', 
                                 'x-axis (g)', 'y-axis (g)', and 'z-axis (g)'.

    Returns:
        pd.DataFrame: Combined DataFrame containing the aligned gyroscope and accelerometer data 
                      with columns 'elapsed (s)', 'gyro_x', 'gyro_y', 'gyro_z', 'accel_x', 'accel_y', and 'accel_z'.
    """
    # Ensure both dataframes are sorted by 'elapsed (s)' for merge_asof to work correctly.
    gyro_df = gyro_df.sort_values(by='elapsed (s)')
    accel_df = accel_df.sort_values(by='elapsed (s)')
    
    # Merge on the nearest elapsed time.
    combined_df = pd.merge_asof(gyro_df, accel_df, on='elapsed (s)', direction='nearest')
    
    # Select and rename the columns.
    combined_df = combined_df[['elapsed (s)', 'x-axis (deg/s)', 'y-axis (deg/s)', 'z-axis (deg/s)', 
                               'x-axis (g)', 'y-axis (g)', 'z-axis (g)']]
    combined_df.columns = ['elapsed (s)', 'gyro_x', 'gyro_y', 'gyro_z', 'accel_x', 'accel_y', 'accel_z']
    
    return combined_df


def segment_data(df, num_segments):
    """Segment the combined data into a specified number of segments.

    This function takes a pandas DataFrame and divides it into the specified number of 
    equal-length segments. If there are any remaining rows that do not evenly divide by the 
    number of segments, they are added to the last segment.
    
    Args:
        df (pd.DataFrame): The DataFrame to be segmented.
        num_segments (int): The number of segments to divide the DataFrame into.

    Returns:
        list of pd.DataFrame: A list containing the segmented DataFrames.
    """
    total_length = len(df)
    segment_length = total_length // num_segments

    # Create the list of DataFrame segments
    segments = [df.iloc[i * segment_length: (i + 1) * segment_length] for i in range(num_segments)]
    
    # If there is any remaining data, add it to the last segment
    if total_length % num_segments != 0:
        remaining_data = df.iloc[num_segments * segment_length:]
        segments[-1] = pd.concat([segments[-1], remaining_data], axis=0)
